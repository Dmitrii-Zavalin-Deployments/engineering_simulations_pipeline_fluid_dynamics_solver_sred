...
def test_advection_step_detected():
    ...
    # Check flux near the step
    inspected = adv[step_x:step_x + 2, 2:3, 2:3]
...
def test_advection_directional_behavior():
    shape = (8, 4, 4)
    field = create_scalar_field_with_step(shape, step_position=4, low=0.0, high=2.0)
    ...
    mesh = {"grid_shape": field.shape, "dx": 1.0, "dy": 1.0, "dz": 1.0}
    adv_fwd = compute_advection_term(field, velocity_fwd, mesh)
    adv_bwd = compute_advection_term(field, velocity_bwd, mesh)
