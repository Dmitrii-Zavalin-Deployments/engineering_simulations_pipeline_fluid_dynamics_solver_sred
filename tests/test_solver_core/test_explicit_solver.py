...
def small_grid_data():
    shape = (4, 4, 4)
    padded_shape = tuple(s + 2 for s in shape)  # Include ghost cells
    dt = 0.01
    viscosity = 0.01
    density = 1.0
    ...
    mesh_info = {
        ...
        "boundary_conditions": {}  # no-op BCs for now
    }
...
def test_step_preserves_zero_velocity(small_grid_data):
    ...
    # Because there's no advection, diffusion, or BCs, it should remain nearly zero
    assert np.allclose(new_u, 0.0, atol=1e-10)

def test_step_decreases_divergence(small_grid_data):
    u, p, mesh_info, fluid_props, dt = small_grid_data
    u[..., 0] = 1.0  # set constant u_x for divergence
    ...
    assert mean_div_after < mean_div_before
