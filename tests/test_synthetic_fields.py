...
import pytest
import numpy as np
from src.stability_utils import check_field_validity, test_velocity_bounds
...
def test_synthetic_corrupted_fields_fail_validation(field_generator, label):
    ...
    result = check_field_validity(field, label=label)
...
def test_malformed_velocity_field_detects_nan():
    ...
    result = check_field_validity(velocity_field, label="MalformedVelocity")
    assert result is False
