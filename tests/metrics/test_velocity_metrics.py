...
def test_varied_velocity_components():
    grid = [
        Cell(x=0, y=0, z=0, velocity=[1.0, 2.0, 2.0], pressure=1.0, fluid_mask=True),  # magnitude ≈ 3.0
        Cell(x=1, y=0, z=0, velocity=[0.0, 3.0, 4.0], pressure=1.0, fluid_mask=True),  # magnitude = 5.0
        Cell(x=2, y=0, z=0, velocity=[2.0, 0.0, 0.0], pressure=1.0, fluid_mask=True)   # magnitude = 2.0
    ]
...
def test_malformed_velocity_vectors():
    ...
    grid = [
        ...
        BadCell(None),
        BadCell([1.0]),
        Cell(x=3, y=0, z=0, velocity=[5.0, 0.0, 0.0], pressure=1.0, fluid_mask=True)
    ]
    assert compute_max_velocity(grid) == 5.0

def test_negative_velocity_components():
    grid = [
        Cell(x=0, y=0, z=0, velocity=[-3.0, 0.0, 0.0], pressure=1.0, fluid_mask=True),     # magnitude = 3.0
        Cell(x=1, y=0, z=0, velocity=[-4.0, -3.0, 0.0], pressure=1.0, fluid_mask=True)     # magnitude = 5.0
    ]
    assert compute_max_velocity(grid) == 5.0

def test_zero_velocity_magnitude():
    grid = [
        Cell(x=0, y=0, z=0, velocity=[0.0, 0.0, 0.0], pressure=1.0, fluid_mask=True),
...
def test_high_precision_magnitude_rounding():
    grid = [
        Cell(x=0, y=0, z=0, velocity=[1.234567, 2.345678, 3.456789], pressure=1.0, fluid_mask=True)
    ]
