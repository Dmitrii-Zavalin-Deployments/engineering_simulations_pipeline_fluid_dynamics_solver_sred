...
def test_pressure_field_changes_if_projected(snapshot, domain, expected_mask, expected_pressure, tolerances):
    ...
    for cell, is_fluid in zip(domain_cells, expected_mask):
        if is_fluid:
            ...
            assert not is_close(actual, expected_pressure, tolerances["pressure"]), \
                f"❌ Pressure did not change: {actual}"
    ...
    for cell, is_fluid in zip(domain_cells, expected_mask):
        if is_fluid:
            ...
            if drift_magnitude < 1e-5:
                print(f"⚠️ Drift too small to register at "
                      f"({cell['x']}, {cell['y']}, {cell['z']}): {velocity} vs {expected_velocity}")
