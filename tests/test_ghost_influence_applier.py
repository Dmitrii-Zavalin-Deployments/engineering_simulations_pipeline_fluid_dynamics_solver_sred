...
class TestGhostInfluenceApplier(unittest.TestCase):
    ...
    def create_cell(self, x, y, z, fluid=True, velocity=None, pressure=None):
        return Cell(
            x=x, y=y, z=z,
            velocity=velocity if velocity else [0.0, 0.0, 0.0],
            pressure=pressure,
            fluid_mask=fluid
        )

    def test_basic_velocity_transfer(self):
        fluid = self.create_cell(1.0, 1.0, 1.0, fluid=True)
    ...
    def test_basic_pressure_transfer(self):
        fluid = self.create_cell(1.0, 1.0, 1.0, fluid=True, pressure=0.0)
    ...
    def test_no_transfer_if_fluid_already_has_values(self):
        fluid = self.create_cell(1.0, 1.0, 1.0, fluid=True, velocity=[1.0, 1.0, 1.0], pressure=50.0)
    ...
    def test_all_fluid_cells_influenced(self):
        fluid1 = self.create_cell(1.0, 1.0, 1.0, fluid=True)
        fluid2 = self.create_cell(2.0, 1.0, 1.0, fluid=True)
