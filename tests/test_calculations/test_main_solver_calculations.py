...
def test_simulation_initialization_and_run(mock_save_snapshot, mock_load, mock_input_data, tmp_path):
    ...
    # Manually set initial velocity & pressure fields
    shape = (4, 4, 4)
    sim.u = np.ones(shape)
    ...
    sim.output_frequency_steps = 1  # ensure snapshot every step

    # Patch time_stepper with dummy evolution
...
def test_missing_boundary_indices_raises_error(mock_load, mock_input_data):
    ...
    broken["mesh_info"] = dict(mock_input_data["mesh_info"])  # shallow copy
    broken["mesh_info"]["boundary_conditions"] = {
        "inlet": {"velocity": [1.0, 0.0, 0.0]}  # no cell_indices
    }
    mock_load.return_value = broken

    with pytest.raises(ValueError, match="missing 'cell_indices'"):
        Simulation("broken.json", "dummy_out")
