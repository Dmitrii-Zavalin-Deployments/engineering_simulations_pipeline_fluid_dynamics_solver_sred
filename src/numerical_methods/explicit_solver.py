...
class ExplicitSolver:
    ...
    def step(self, velocity_field: np.ndarray, pressure_field: np.ndarray, should_log_verbose: bool) -> tuple[np.ndarray, np.ndarray]:
        # Only print the step start/end if verbose logging is enabled
        ...
        advection_u = compute_advection_term(velocity_field[..., 0], velocity_field, self.mesh_info, should_log_verbose=should_log_verbose)
        advection_v = compute_advection_term(velocity_field[..., 1], velocity_field, self.mesh_info, should_log_verbose=should_log_verbose)
        advection_w = compute_advection_term(velocity_field[..., 2], velocity_field, self.mesh_info, should_log_verbose=should_log_verbose)

        u_star[..., 0] -= self.dt * advection_u
        u_star[..., 1] -= self.dt * advection_v
        u_star[..., 2] -= self.dt * advection_w

        diffusion_u = compute_diffusion_term(u_star[..., 0], self.viscosity, self.mesh_info, should_log_verbose=should_log_verbose)
        diffusion_v = compute_diffusion_term(u_star[..., 1], self.viscosity, self.mesh_info, should_log_verbose=should_log_verbose)
        diffusion_w = compute_diffusion_term(u_star[..., 2], self.viscosity, self.mesh_info, should_log_verbose=should_log_verbose)
        ...
        # Critical warnings should not be suppressed by verbose flag
        ...
        u_star, _ = apply_boundary_conditions(
            ...
            pressure_field=pressure_field, # Pressure field might not be directly used here for tentative vel BCs, but passed for consistency
            fluid_properties=self.fluid_properties_dict,
            mesh_info=self.mesh_info,
            is_tentative_step=True,
            should_log_verbose=should_log_verbose # Pass the flag
        )

        if should_log_verbose:
            print("  3. Solving Poisson equation for pressure correction...")
        # compute_pressure_divergence also needs should_log_verbose
        divergence = compute_pressure_divergence(u_star, self.mesh_info, should_log_verbose=should_log_verbose)
        ...
        if should_log_verbose: # Only print this if verbose
        ...
        self.step_counter += 1
        # log_flow_metrics now directly accepts and uses should_log_verbose
        log_flow_metrics(
            velocity_field=updated_velocity,
            pressure_field=updated_pressure,
            divergence_field=divergence, # This is the divergence of u*, not the final velocity.
            fluid_density=self.density,
            step_count=self.step_counter,
            current_time=self.step_counter * self.dt,
            should_log_verbose=should_log_verbose # Pass the flag here
        )
