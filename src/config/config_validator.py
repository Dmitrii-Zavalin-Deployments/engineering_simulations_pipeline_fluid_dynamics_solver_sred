...
def validate_config(config: Dict) -> None:
    ...
    for key in ["min_x", "max_x", "min_y", "max_y", "min_z", "max_z"]:
        if key not in domain or not isinstance(domain[key], (int, float)):
            raise ValueError(
                f"Missing or invalid '{key}' in 'domain_definition'."
            )
    ...
    if ghost_rules:
        ...
        if (
            "boundary_faces" not in ghost_rules
            or not isinstance(ghost_rules["boundary_faces"], list)
        ):
            raise ValueError(
                "Missing or invalid 'boundary_faces' in 'ghost_rules'."
            )
        if (
            "default_type" not in ghost_rules
            or not isinstance(ghost_rules["default_type"], str)
        ):
            raise ValueError(
                "Missing or invalid 'default_type' in 'ghost_rules'."
            )
        if (
            "face_types" not in ghost_rules
            or not isinstance(ghost_rules["face_types"], dict)
        ):
            raise ValueError(
                "Missing or invalid 'face_types' in 'ghost_rules'."
            )
    ...
    if not isinstance(boundary_conditions, list):
        raise ValueError(
            "Missing or invalid 'boundary_conditions' — must be a list."
        )

    for i, bc in enumerate(boundary_conditions):
        if not isinstance(bc, dict):
            raise ValueError(
                f"boundary_conditions[{i}] must be a dictionary."
            )
        if (
            "apply_to" not in bc
            or not isinstance(bc["apply_to"], list)
        ):
            raise ValueError(
                f"boundary_conditions[{i}] missing or invalid 'apply_to' list."
            )
        if (
            "type" not in bc
            or not isinstance(bc["type"], str)
        ):
            raise ValueError(
