...
# 🌀 Initial Field Assigner — seeds velocity and pressure for ∂u/∂t initialization
# and reflex diagnostics
...
def assign_fields(cells: list[Cell], initial_conditions: dict) -> list[Cell]:
    ...
    if (
        ...
        not all(isinstance(v, (int, float)) for v in velocity)
    ):
        raise ValueError(
            "❌ 'initial_velocity' must be a list of 3 numeric components"
        )
    ...
    for cell in cells:
        ...
        if fluid:
            ...
            if debug:
                print(
                    f"[INIT] Fluid @ ({cell.x:.2f}, {cell.y:.2f}, {cell.z:.2f}) → "
                    f"velocity: {velocity}, pressure: {pressure}"
                )
        else:
            cell.velocity = None
            cell.pressure = None
